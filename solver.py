...
class Solver(object):
    ...
    def build_model(self):
        ...
        self.D = Discriminator(self.image_size, self.d_conv_dim, self.r_dim, self.d_repeat_num) 
    ...
    def create_reg_labels(self, r_org):
        ...
        for center in centers:
            r_trg = torch.tensor([center] * r_org.size(0))
    ...
    def train(self):
        ...
        for i in range(start_iters, self.num_iters):
            ...
            try:
                x_real, va_org = next(data_iter) # NCHW, N1, N2
            except:
                data_iter = iter(data_loader)
                x_real, va_org = next(data_iter)

            # Generate target domain labels randomly.
            rand_idx = torch.randperm(va_org.size(0))
            ...
            out_src, out_reg = self.D(x_real)
            d_loss_real = - torch.mean(out_src)
            d_loss_reg = self.regression_loss(out_reg, va_org)

            # Compute loss with fake images.
            x_fake = self.G(x_real, r_trg)
            out_src, out_reg = self.D(x_fake.detach())
            ...
            d_loss = d_loss_real + d_loss_fake + self.lambda_reg * d_loss_reg + self.lambda_gp * d_loss_gp
            self.reset_grad()
            d_loss.backward()
            ...
            if (i+1) % self.n_critic == 0:
                ...
                x_fake = self.G(x_real, r_trg)
                out_src, out_reg = self.D(x_fake)
                g_loss_fake = - torch.mean(out_src)
                g_loss_reg = self.regression_loss(out_reg, va_trg)

                # Target-to-original domain.
                x_reconst = self.G(x_fake, r_org)
                g_loss_rec = torch.mean(torch.abs(x_real - x_reconst))

                # Backward and optimize.
                g_loss = g_loss_fake + self.lambda_rec * g_loss_rec + self.lambda_reg * g_loss_reg
                self.reset_grad()
                g_loss.backward()
            ...
            if (i+1) % self.sample_step == 0:
                with torch.no_grad():
                    ...
                    for r_fixed in r_fixed_list:
                        x_fake_list.append(self.G(x_fixed, r_fixed))
    ...
    def test(self):
        ...
        with torch.no_grad():
            for i, (x_real, r_org) in enumerate(data_loader):
