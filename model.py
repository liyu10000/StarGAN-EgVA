...
class Generator(nn.Module):
    """Generator network."""
    def __init__(self, conv_dim=64, r_dim=2, repeat_num=6):
        super(Generator, self).__init__()

        layers = []
        layers.append(nn.Conv2d(3+r_dim, conv_dim, kernel_size=7, stride=1, padding=3, bias=False))
    ...
    def forward(self, x, r):
        ...
        x = torch.cat([x, r], dim=1)              # N,C+2,H,W
        return self.main(x)


class Discriminator(nn.Module):
    """Discriminator network with PatchGAN."""
    def __init__(self, image_size=128, conv_dim=64, r_dim=2, repeat_num=6):
    ...
    def forward(self, x):
        ...
        return out_src, out_reg.view(out_reg.size(0), out_reg.size(1))
